...
import { calculateProgress, calculateRemaining, calculateStatus, formatMeters } from '../utils'
...
function BaseModal({ open, title, onClose, children }: BaseModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-card bg-white p-4 shadow-lg">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="m-0 text-lg font-semibold">{title}</h3>
          <button type="button" className="rounded px-2 py-1 text-sm" onClick={onClose}>
            关闭
          </button>
        </div>
        {children}
  ...
  return (
    <BaseModal open={open} title={value ? '编辑布料' : '添加布料'} onClose={onClose}>
      <form className="space-y-3" onSubmit={onFormSubmit}>
        ...
        <label className="block text-sm">
          布料来源
          <input className="mt-1 w-full rounded border p-2" value={source} onChange={(e) => setSource(e.target.value)} />
        </label>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" className="rounded bg-primary px-3 py-2 text-white">
            保存
          </button>
        </div>
      </form>
...
interface ItemFormModalProps {
  ...
  onClose: () => void
  onSubmit: (payload: { name: string; source?: string; imageBase64: string }) => void
}
    ...
    <BaseModal open={open} title={title} onClose={onClose}>
      <form
        ...
        onSubmit={(e) => {
          ...
          if (!imageBase64) return alert('请上传图片')
          onSubmit({ name: name.trim(), source: showSource ? source.trim() || undefined : undefined, imageBase64 })
        }}
      >
        <label className="block text-sm">
          名称 *
      ...
      <form
        ...
        onSubmit={(e) => {
          ...
          if (entityType === 'pattern') {
            onSubmit({
              entityType: 'pattern',
              data: { name: name.trim(), source: source.trim() || undefined, imageBase64 },
            })
            return
          }
      ...
      >
        <label className="block text-sm">
          ...
          >
            ...
            <option value="pattern">✂️ 纸样</option>
        ...
        {isFabric ? (
          <>
            <label className="block text-sm">
              布料类型 *
              <input className="mt-1 w-full rounded border p-2" value={fabricType} onChange={(e) => setFabricType(e.target.value)} />
            </label>
            <label className="block text-sm">
              布料来源
              <input className="mt-1 w-full rounded border p-2" value={source} onChange={(e) => setSource(e.target.value)} />
...
export function FabricCard({ fabric, onEdit, onDelete, onUse }: FabricCardProps) {
  ...
  return (
    <article className="rounded-card bg-white p-3 shadow">
      ...
      <div className="mt-3 flex gap-2">
        <button type="button" className="rounded border px-3 py-1.5 text-sm" onClick={onUse}>
          使用
        </button>
        <button type="button" className="rounded border px-3 py-1.5 text-sm" onClick={onEdit}>
          编辑
        </button>
        <button type="button" className="rounded border px-3 py-1.5 text-sm text-red-600" onClick={onDelete}>
          删除
        </button>
      </div>
    </article>
...
export function GridCard({ title, source, imageBase64, onEdit, onDelete }: GridCardProps) {
  return (
    <article className="rounded-card bg-white p-3 shadow">
